    ...
    <view v-if="isShow404">
        ...
        <view class="card p-lr-20 m-tb-20 p-bottom-20">
            <view class="top p-top-30">
                <view :class="index === 0 ? 'top_view-active' : 'top_view_left'" @click="left(0)">任务介绍</view>
                <view :class="index === 1 ? 'top_view-active' : ' top_view_right'" @click="left(1)">成员业绩</view>
                <view :class="index === 2 ? 'top_view-active' : ' top_view_right'" @click="left(2)">团队业绩</view>
            </view>
            <view class="main p-top-20" v-if="index === 0">
                ...
                <view class="item">任务周期：{{$minCommon.formatDate(new Date(list.mission_start_time * 1000),'yyyy/MM/dd hh:mm:ss')}}至{{$minCommon.formatDate(new Date(list.mission_end_time * 1000),'yyyy/MM/dd hh:mm:ss')}}
                </view>
                <view class="item">任务状态：{{ status[list.status] }}</view>
                <view class="item">分红比例：{{ flag ? list.mission_ratio + '%' : '****' }}
                    <image style="width:30rpx;height:30rpx;margin-left:10rpx" @click="eye" :src="flag ? '/static/images/eyes.png' : '/static/images/eyes_.png'" />
                </view>
            </view>

            <view class="main p-top-20" v-if="index === 1 || index === 2">
                <view class="item_table" v-for="(n, i) in users" :key="i">
                    <view :class="i === 0 ? 'num status confirmed' : i === 1 ? 'num status end' : i === 2 ? 'num status force' : 'num' ">{{ i + 1 }}</view>
                    <view :class="i === 5 ? 'content' : 'content min-border-bottom'">
                        <view style="width:130rpx">{{ n.user_name }}</view>
                        <view style="width:33%;text-align:center">{{ n.ident }}</view>
                        <view style="width:33%;text-align:right">{{ n.achievement }}</view>
                    </view>
                </view>
            </view>
        </view>
