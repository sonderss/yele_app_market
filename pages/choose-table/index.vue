<template>
  <view class="choose-table p-tb-20 p-lr-30">
    <min-pcitem :isBorder="false" :list="$parseURL().item" @emitE="toEmint($parseURL().item)" />
    <view class="m-tb-20"></view>
    <min-cell :card="false">
      <min-cell-item :border="false" :title="`时间: ${dates}`" arrow @eventParent="toChinceDate"></min-cell-item>
    </min-cell>
    <view
      class="tesk"
      :style="{background:`url(${getNum(n)})`,backgroundSize:'cover'}"
      v-for="(i,n) in deskGroup"
      :key="i.id"
      @click="chinceGrop(n)"
    >
...
<script>
const bgs = [
  '/static/images/desk/0.png',
  '/static/images/desk/1.png',
  '/static/images/desk/2.png',
  '/static/images/desk/3.png',
]
export default {
  name: 'choose-table',
  navigate: ['navigateTo'],
  ...
  methods: {
    ...
    chinceGrop(n) {
      this.current = n
      if (this.deskGroup[n].is_full === 1)
        return this.$showToast('没有空闲台!!!')
      this.$minRouter.push({
        name: 'reservation',
        params: {
          store: this.$parseURL().item,
          group: this.deskGroup[n],
          date: this.dates,
        },
      })
    },
