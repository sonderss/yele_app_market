...
<view class="commission p-tb-20 p-lr-30">
    <view class="view_main p-lr-20 p-bottom-20">
        ...
        <view class="bottom-view">
            ...
            <view class="main min-flex min-flex-main-between">
                ...
                <text style="text-align:right">{{$minCommon.formatDate(new Date(list.commission_create_time*1000),'yyyy/MM/dd hh:mm:ss') }}</text>
            </view>
        </view>
    </view>
    <!-- 活动信息   list.commission_type !== 2 || list.commission_type !== 1 || list.commission_type !== 3 || list.commission_type !== 5-->
    <view class="card p-lr-20 p-bottom-10 m-bottom-20" v-if="list.commission_type !== 2 && list.commission_type !== 1 && list.commission_type !== 3 && list.commission_type !== 5">
    ...
    <!-- 提成信息 -->
    <view class="card p-lr-20 p-bottom-10 m-bottom-20" v-if="list.commission_type === 1 || list.commission_type === 5">
        <view class="p-tb-30 min-border-bottom q">提成信息</view>
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
            <text class="f28">业绩</text>
            <text class="f26">{{list.performance_total }}</text>
        </view>
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
            <text class="f28">提成</text>
            <text class="f26">{{list.prepay_ratio}}</text>
        </view>
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
        ...
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
            ...
            <text class="f26">{{list.prepay_amount}}</text>
        </view>
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
            <text class="f28">扣减线索分佣</text>
            <text class="f26">{{list.rebate_amount }}</text>
        </view>
        <view class="m-bottom-10 m-top-20 t" style="width:100%;display: flex;justify-content: space-between;">
            <text class="f28">实得提成</text>
            <text class="f26">{{list.actual_commission }}</text>
        </view>
    </view>
