...
<view class="min-pcitem p-lr-20" style="  background: rgba(255, 255, 255, 1);">
    <view :class="isBorder ? 'main-item min-border-bottom':'main-item'" @click="toEmit">
        ...
        <view class="main p-bottom-20">
            <view class="f30" v-if="desk">{{list.store_name}}</view>
            <view class="f30" v-if="!desk">
                {{list.store_name}} |
                <text class="f24 m-left-10" style="color: #666666;">营业:{{list.store_config.business_start_time}}--{{list.store_config.business_end_time}}</text>
        ...
        <view class="right_view" v-if="showArray">
            <image style="width:24rpx;height:24rpx;" class="arrow" src="/static/images/arrow.png" />
        </view>
    </view>
</view>
</template>
